...
zeroes <- function(a,b,c) {
      ...
      if (roots < 0){
              ...
              realroots <- c((-b+sqrt(roots))/(2*a),(-b-sqrt(roots))/(2*a))
              return(realroots)
      }
}
zeroes(1, 0, 0)
...
max(b)
```
pmax returns the parallel maxium of the vectors. on the other hand, max gives only maxium of the vectors, giving a single variable that is maximum of the vectors.
...
AdvRoll <- function(sides,n) {
        first_roll <- sample(sides,n, replace = TRUE)
        second_roll <- sample(sides,n,replace = TRUE)
        pmax(first_roll,second_roll)
}
AdvRoll(12,10)
...
Mysummary <- function(x) {
        list(mean = mean(x),
             median = median(x),
             mode = as.numeric(names(sort(table(x), T))[1]),
       sum = sum(x),
       sum_of_squares = (sum(x)^2))
}
Mysummary(c(1,1,2,3,4))
```
