...
# Problem 1
files100 <- c(
        paste("plot00",1:9, ".png", sep = ""),
        paste("plot0",10:99, ".png", sep = ""),
        paste("plot",100,".png", sep = "")
)
files100
...
```{r}
length(grep("i", states, ignore.case = T))
```

2. How many states do not contain the letter `a`?
```{r}
 acoun <- sum(table(str_subset(states, ("a"))))
 acoun
 notacoun <- 50 - acoun
 notacoun
...
tiii <- c("[i].*[i].*[i]")
str_subset(tolower(states),tiii) 
...
states[str_count(tolower(states), "[aeiou]")==5]
...
```{r}
  ...
  vowel <- "[aeiou]{3}"
 str_subset(tolower(states), vowel)
```
...
# is_hex()
is_hex <- function(x) {
       hexx <- grep("^#[0123456789abcdef]{6}$",tolower(x),value = T)
       length(hexx) == 1
...
reverse_chars <- function(x) {
        spliter <- strsplit(as.character(x), split = "")
        reversed <- paste((spliter[[1]][nchar(x):1]), collapse = "")
     return(reversed)
}
...
remove_tag <- function(x) {
       ...
       frontrmv <- str_replace_all(x,"^[<][a-z]+[>]", replacement = "")
       return(str_replace_all(frontrmv,"[<][/][a-z]+[>]$", replacement = ""))
...
tripletss <- codons$dna_triplet
aminoac <- codons$amino_acid
names(aminoac) <- codons$dna_triplet
dnaseq <- substring(dna,seq(1,nchar(dna),3),seq(3,nchar(dna),3))
dector <-str_detect(substring(dna,seq(1,nchar(dna),3),seq(3,nchar(dna),3)),"^[CGAT]{3}")
if (all(dector == T)) {
      dnaseq <- str_replace_all(dnaseq, aminoac)
      dnaseq <- str_c(dnaseq, collapse = "")
      print(dnaseq)
} else {
        print("Invaild DNA sequence")
}
}
```
